...
# Combine into 1:
search_polys <- st_sf(
  area_poly = c("German Seal", "Scots Bay"),
...
for (f in files) {
  ...
  inside_germanseal <- inside_polygon[, search_polys$area_poly == "German Seal"]
  ...
  cat("German Seal polygon casts:", length(idx_germanseal), "\n")
  ...
  for (i in idx) {
    ...
    area_i <- case_when(
      ...
      i %in% idx_germanseal ~ "German Seal",
    ...
    # legend
    legend("topright",
           legend = c("Prince-5", "German Seal polygon", "Scots Bay polygon", "Current cast"),
...
cast_averages <- final_casts_deep %>%
  group_by(file, cast, date, station_id, latitude, longitude, area) %>%
  summarise(
...
monthly_cast_averages <- cast_averages %>%
  group_by(year, month, month_name, station_id, area) %>%
  summarise(
...
write.csv(
  ...
  "DFO_monthly_cast_averages_between5-30.csv",
  row.names = FALSE
)
