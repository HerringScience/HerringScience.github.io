...
#QC
length(unique(HSC$id))
# 234 HSC casts

ids_deeper_than_1m <- HSC %>%
  group_by(id) %>%
  summarise(max_depth = max(Depth, na.rm = TRUE)) %>%
  filter(max_depth > 1) %>%
  pull(id)

length(ids_deeper_than_1m)

# find casts that only have 1 observation


length(single_obs_ids)

231-117

#114 HSC casts with data


# Exports a df with all the casts that have either absent temperature or salinity data:
# turns out it's all historic DFO data, most have temp but no salinity 
Oceans_flagged <- Oceans %>%
  group_by(id) %>%
  mutate(
    bad_cast = all(is.na(Temperature)) | all(is.na(Salinity))
  ) %>%
  ungroup()

write.csv(Oceans_flagged, "Oceans_flagged.csv", row.names = FALSE)
...
setwd("C:/Users/herri/Documents/GitHub/HerringScience.github.io/Source Data/CTD/_Scripts/Analyses")
