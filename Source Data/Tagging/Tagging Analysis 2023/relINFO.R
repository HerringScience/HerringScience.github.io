...
#### Produce all tag return DF's ####
  ...
  # make sure there are no duplicate tag #s in rel
    ...
    # Total number of tags applied between 2016-2023 (8 years)
      sum(relINFO$no_tags)  
      #158,723
      ...
      n_occur <- data.frame(table(rel$TAG_NUMBER))
      duplicated  = data.frame(n_occur[n_occur$Freq > 1,])
      
      write.table(duplicated, file= "duplicatedTags.csv", sep = ",", quote=FALSE, row.names=FALSE, col.names = .names=TRUE) 
...
# Ground                        
            ...
            head(re)             
                  ...
                  sub = data.frame(relINFO$set,relINFO$X, relINFO$Y) 
                  
                  colnames(sub) = c("set", "X", "Y")
                  head(sub)
                  
                 groundsEvents =  inner_join(`grounds`, `sub`, by = 'set')
...
# Plot each set on a map, colour code by ground and then also display grounds as a quality control
                    ...
                    ggplot(groundsEvents, aes(x=X, y=Y)) +geom_polygon(data=polysT,aes(x=X, y=Y, group=Box, fill = Box), colour = "black") + geom_polygon(data=out,aes(x=long, y=lat, group=group),fill='tan4',col='black') + coord_map() + geom_point(aes(fill = FishingGround), pch = 22)
                    
                    
                    # something weird going on Grand Manan banks, also a point that is on land
                    
                 head(groundsEvents)
                    
                    
                    # missing the scotian shelf tagging - asked Allan for coordinates Nov 14. These tagging events are missing from the map.
                  ...
                  + geom_segment(aes(x = X, y = Y, xend = Xend, yend = Yend, colour = Vessel), size = 1)  + labs(x=NULL, y=NULL) + coord_map() + theme_dark()
