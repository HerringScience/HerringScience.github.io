...
write.table(retRel, file= "retRel.csv", sep = ",", quote=FALSE, row.names=FALSE, col.names=TRUE) 
...
grounds= subset(polysT, !(polysT$Box %in% ids))
unique(grounds$Box)

save(grounds, file="grounds.RData", compress=T) 

# remove Browns Bank and SW grounds for mapping

# look at tagging events by grounds
        ggplot(relINFO, aes(x=X, y=Y)) + geom_polygon(data=polysT,aes(x=X, y=Y, group=Box, fill = Box), colour = "black") + geom_polygon(data=out,aes(x=long, y=lat, group=group),fill='burlywood4',col='black')  + geom_point(colour = "white") + labs(x=NULL, y=NULL) + coord_map() + theme(panel.background = element_rect(fill = "grey68")) + ggtitle("Tagging Events by Ground") + scaleBar(lon = -64, lat = 43.5, distanceLon = 25, distanceLat = 5, distanceLegend = -4, dist.unit = "km")
...
# look at tagging events by grounds
ggplot(returnDF, aes(x=X, y=Y)) + geom_polygon(data=polysTee,aes(x=X, y=Y, group=Box, fill = Box), colour = "black") + geom_polygon(data=out,aes(x=long, y=lat, group=group),fill='burlywood4',col='black')  + geom_point(colour = "white") + labs(x=NULL, y=NULL) + coord_map() + theme(panel.background = element_rect(fill = "grey68")) + ggtitle("Tag Returns by Ground") + scaleBar(lon = -64, lat = 43.5, distanceLon = 25, distanceLat = 5, distanceLegend = -4, dist.unit = "km")

# By NAFO subunit

ggplot(returnDF, aes(x=X, y=Y)) + geom_polygon(data=polysNAFO,aes(x=X, y=Y, group=Area, fill = Area), colour = "black") + geom_polygon(data=out,aes(x=long, y=lat, group=group),fill='burlywood4',col='black')  + geom_point(colour = "white") + labs(x=NULL, y=NULL) + coord_map() + theme(panel.background = element_rect(fill = "grey68")) + ggtitle("Tagging Events by NAFO Subunit") + scaleBar(lon = -64, lat = 43.5, distanceLon = 25, distanceLat = 5, distanceLegend = -4, dist.unit = "km")


# Look at annual differences in the number of tags returns

year = with(returnDF, tapply(no, list(Year), sum))
year = as.data.frame(year)
x = rownames(year)
year$Year  =  x
...
# Look at temporal differences in the number of tags returns

month = with(returnDF, tapply(no, list(month), sum))
month = as.data.frame(month)
x = rownames(month)
month$Month  =  x
