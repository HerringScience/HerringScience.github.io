...
# scale bar function: three functions
# createScaleBar
...
createOrientationArrow <- function(scaleBar, length, distance = 1, dist.units = "km"){ 
  ...
  res <- rbind(cbind(x = lon, y = lat, xend = onTop[1,"long"], yend = onTop[1,"lat"]),cbind(x = leftArrow[1,"long"], y = leftArrow[1,"lat"], xend = onTop[1,"long"], yend = onTop[1,"lat"]),cbind(x = rightArrow[1,"long"], y = rightArrow[1,"lat"], xend = onTop[1,"long"], yend = onTop[1,"lat"]))
...
scaleBar <- function(lon, lat, distanceLon, distanceLat, distanceLegend, dist.unit = "km", rec.fill = "white", rec.colour = "black", rec2.fill = "black", rec2.colour = "black", legend.colour = "black", legend.size = 2, orientation = TRUE, arrow.length = 25, arrow.distance = 5, arrow.North.size = 5){

  # legend size changes the size of the numbers at the bottom, i.e. 0km, 35km, 70km
  # arrow.North.size changes the size of the N
  # arrow.distance changes the distance of the N/arrow with the rectangle
  # arrow.length changes the length of the arrow (should be larger than the arrow.North.size)
  # orientation controls where you want the arrow and N prsent on your figure
  

#scaleBar <- function(lon, lat, distanceLon, distanceLat, distanceLegend, dist.unit = "km", rec.fill = "white", rec.colour = "black", rec2.fill = "black", rec2.colour = "black", legend.colour = "black", legend.size = 3, orientation = TRUE, arrow.length = 10, arrow.distance = 15, arrow.North.size = 10){
