...
if(surv=="GB"){Annual_Compare1 = SSB %>%
  filter(Year == year & Ground == "German Bank" & Survey_Number == 1 | Year == year & Ground == "Seal Island" & Survey_Number == 1) %>%
  dplyr::select(-DFO_Estimate, -DFO_Turnover_Adjusted)
...
```{r, results = "hide", fig.show="first", fig.align='center'}
a=ggplot(data = subset(CTD2, Ground == "Scots Bay" & Year == year & Survey == 1), aes(x=Temperature, y=Depth, colour = Julian_factor)) + 
geom_path(size = 1.5) + 
scale_y_reverse() + 
labs(x="Temperature (°C)", y = "Depth (m)", colour='Julian Day', title = "Temperature (°C) vs. Depth (m)")

b=ggplot(data = subset(CTD2, Ground == "German Bank" & Year == year & Survey == 1), aes(x=Temperature, y=Depth, colour = Julian_factor)) + 
geom_path(size = 1.5) + 
scale_y_reverse() + 
labs(x="Temperature (°C)", y = "Depth (m)", colour='Julian Day', title = "Temperature (°C) vs. Depth (m)")
...
SST <- SST %>% filter(Date < '2023-08-07')
SST <- SST %>% filter(Ground == 'German Bank')
