...
# Survey plan
              track = read.csv("july7_plan.csv", header=TRUE, sep=",", row.names = NULL, stringsAsFactors=FALSE)
              
              # Scot's Bay
              ggplot(track,aes(x=X1, y=Y2)) + geom_polygon(data=out,aes(x=long, y=lat, group=group)) + geom_polygon(data = plankton, aes(x=X, y=Y), fill = "white", colour = "black") + geom_segment(aes(x = X, y = Y, xend = Xend, yend = Yend, colour = Vessel), size = 1) + ggtitle("Scots Bay Survey Plan - July 7, 2018") + coord_map() + labs(x=NULL, y=NULL)
...
trans_survey2=trans_survey1[which(trans_survey1$Vessel != "BP"), ]
...
resultsc = biomassCalc(x = eastern, areaKm = 127.37)
