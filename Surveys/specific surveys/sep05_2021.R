...
SUA = read.csv("polygon_SI2021_Sep05.csv")
...
# September 5, 2021 German Banks
regions = read.table("Region_Sep05_2021.csv", header=TRUE, sep=",", row.names = "id", stringsAsFactors=FALSE)

mapping = read.table("Map_Sep05_2021.csv", header=TRUE, sep=",", row.names = "id", stringsAsFactors=FALSE)
...
x = surveyTrack3(x=trans, polyNameA  = polyGB_Ran,polyNameB  = polySI, polyNameC  = polyGB, title = name )
...
x = surveyTrack(x=trans_surveySI,polyNameA  = polySI)
...
trans_survey= trans[which(trans$Survey_date == "Sep05_2021"), ]
...
# remove BP
trans_surveySI=trans_surveySI[which(trans_surveySI$Vessel != "BP"), ]
...
resultsb = biomassCalc(x = trans_surveySI, areaKm = 241.7)
