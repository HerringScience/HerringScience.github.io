...
# Proper coordinates for ad hoc
CP <- as(extent(-66.5, -66, 43, 43.4), "SpatialPolygons")
...
x =c(-66.45433333,-66.45016667, -66.16666667, -66.165, -66.13416667, -66.46606667,-66.3665, -66.365 ) 
y =c(43.291,43.28733333,43.33833333, 43.355, 43.33,43.30836667, 43.5819, 43.561) 
no_tags = c(370, 456, 124, 50, 99, 150, 100, 100)
vessel = c("SealifeII", "SealifeII", "SealifeII", "SealifeII", "SealifeII", "LadyJanice", "LadyMelissa", "LadyMelissa") 
type= c("tagging","tagging","tagging","tagging","tagging","tagging", "CTD", "plankton")
sum(no_tags)
...
# October 3, 2021 German Bank
regions = read.table("Region_Oct3_2021.csv", header=TRUE, sep=",", row.names = "id", stringsAsFactors=FALSE)

regions = read.table("Region_Oct3_2021_.csv", header=TRUE, sep=",", row.names = "id", stringsAsFactors=FALSE)

regions = read.table("Region_Oct05_2021.csv", header=TRUE, sep=",", row.names = "id", stringsAsFactors=FALSE)


mapping = read.table("Map_Oct05_2021.csv", header=TRUE, sep=",", row.names = "id", stringsAsFactors=FALSE)


mapping = read.table("Map_Oct3_2021.csv", header=TRUE, sep=",", row.names = "id", stringsAsFactors=FALSE)

mapping = read.table("Map_Oct3_2021_.csv", header=TRUE, sep=",", row.names = "id", stringsAsFactors=FALSE)
...
SUA = read.csv("polygonGB_Oct5AH.csv")
polyGB = as.PolySet(SUA, projection="LL")
x = surveyTrack(x=trans, polyNameA  = polyGB)


SUA = read.csv("polygonSI_Oct5AH.csv")
polySI = as.PolySet(SUA, projection="LL")
...
trans_survey= trans[which(trans$Survey_date == "Oct03_2021"), ]
trans_survey= trans[which(trans$Survey_date == "Oct05_2021"), ]
...
# Ad hoc:
trans_survey$Transect_No
#  GB:T1-5

ids = c("T01", "T02","T03", "T04", "T05")

trans_surveyGB=trans_survey[which(trans_survey$Transect_No == ids), ]

# SI:T6-8
trans_surveySI=trans_survey[which(trans_survey$Xend > -66.3), ]
...
resultsa = biomassCalc(x = trans_surveyGB, areaKm = 3.9)
unique(resultsa$total_biomass)
#15,722

#4,476

resultsb = biomassCalc(x = trans_surveySI, areaKm = 265)
unique(resultsb$total_biomass)

#1,286


resultsb = biomassCalc(x = trans_surveySI, areaKm = 3.2)
unique(resultsb$total_biomass)
