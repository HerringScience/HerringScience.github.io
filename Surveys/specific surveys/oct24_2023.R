...
# GB 
regions = read.table("Region_Oct24_2023.csv", header=TRUE, sep=",", row.names = "id", stringsAsFactors=FALSE)

mapping = read.table("Map_Oct24_2023.csv", header=TRUE, sep=",", row.names = "id", stringsAsFactors=FALSE)
...
trans_survey= trans[which(trans$Survey_date == "Oct24_2023"), ]
...
head(trans)
ids = c("T03", "T04")
SI = trans[which(trans$Transect_No %in% ids), ]
