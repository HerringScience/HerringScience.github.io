...
# November 2, 2021 German Bank and Seal Island
...
x =c(-66.35416667, -66.35721667) 
y =c(43.21833333, 43.23851667) 
no_tags = c(100, 101)
vessel = c("LM","LJ") 
type= c("Tagging", "Tagging")
sum(no_tags)

df = data.frame(x,y,no_tags,vessel)

ggplot(df, aes(x=x, y=y)) + geom_polygon(data=out,aes(x=long, y=lat, group=group),fill='tan4',col='black') + geom_polygon(data=polyGB_Ran,aes(x=X, y=Y, group=PID), fill = "yellow", colour = "black") +geom_polygon(data=polySI,aes(x=X, y=Y, group=PID), fill = "white", colour = "black")+ geom_point(aes(colour = vessel), size = 5) + labs(x=NULL, y=NULL) + coord_map() + theme_dark()
...
# November 2, 2021 German Bank
regions = read.table("Region_Nov02_2021.csv", header=TRUE, sep=",", row.names = "id", stringsAsFactors=FALSE)

mapping = read.table("Map_Nov02_2021.csv", header=TRUE, sep=",", row.names = "id", stringsAsFactors=FALSE)
...
trans_survey= trans[which(trans$Survey_date == "Nov02_2021"), ]
trans_survey= trans[which(trans$Survey_date == "Nov02_2021"), ]
