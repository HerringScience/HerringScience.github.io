...
# GB 
regions = read.table("Region_Sep27_2023.csv", header=TRUE, sep=",", row.names = "id", stringsAsFactors=FALSE)

mapping = read.table("Map_Sep27_2023.csv", header=TRUE, sep=",", row.names = "id", stringsAsFactors=FALSE)
...
trans_survey= trans[which(trans$Survey_date == "Sep27_2023"), ]
...
SI = trans[which(trans$Vessel %in% id), ]

calcArea(polySI) 

#272

ids = c("BP", "C1", "FM", "LB", "LJ", "LM", "MS")

GB = trans[which((trans$Vessel %in% ids)), ]
