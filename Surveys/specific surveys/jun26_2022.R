...
#June 26 2022
regions = read.table("Region_Jun26_2022.csv", header=TRUE, sep=",", row.names = "id", stringsAsFactors=FALSE)

mapping = read.table("Map_Jun26_2022.csv", header=TRUE, sep=",", row.names = "id", stringsAsFactors=FALSE)
...
trans = transects(x= regions, TS38 = -35.01, TS50 = NA )
...
ids = c("LJ","LB", "FM", "TM", "BP")

northern = trans[which((trans$Vessel == 'SL')), ]
eastern = trans[which((trans$Vessel == 'C1')), ]
...
# DFO Area Results
resultsa = biomassCalc(x = main, areaKm = 640)
a = unique(resultsa$total_biomass)

resultsb = biomassCalc(x = northern, areaKm = 82)
b = unique(resultsb$total_biomass)
