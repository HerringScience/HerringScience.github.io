...
# GB 
regions = read.table("Region_Sep24_LB_2023.csv", header=TRUE, sep=",", row.names = "id", stringsAsFactors=FALSE)

mapping = read.table("Map_Sep24_LB_2023.csv", header=TRUE, sep=",", row.names = "id", stringsAsFactors=FALSE)

map = mapDat(x = mapping)
x = regions
trans = transects(x= regions, TS38 = -35.5, TS50 = NA )

SUA = read.csv("polygon_SI_LB.csv")
polySI = as.PolySet(SUA, projection="LL")

x = surveyTrack(x=trans, polyNameA  = polySI,  title = name )
...
trans_survey= trans[which(trans$Survey_date == "Sep24_2023"), ]
...
resultsa = biomassCalc(x = trans, areaKm = 20)
