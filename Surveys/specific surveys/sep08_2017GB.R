...
# Sep 08, 2017 German Bank
regions1 = read.table("RegionSep08GB.csv", header=TRUE, sep=",", row.names = "id", stringsAsFactors=FALSE)

mapping1 = read.table("MapSep08GB.csv", header=TRUE, sep=",", row.names = "id", stringsAsFactors=FALSE)

trans1 = transects(x= regions1,TS50 = -34.737, TS38 = -34.630)
# Sep 8, 2017
#x$TS<-ifelse((x$Frequency>38), -34.737, -34.630)

# Load school height data

pelagic = read.table("Pelagic_Sep08.csv", header=TRUE, sep=",", row.names = "id", stringsAsFactors=FALSE)

demersal = read.table("Demersal_Sep08.csv", header=TRUE, sep=",", row.names = "id", stringsAsFactors=FALSE)
...
trans_survey= trans1[which(trans1$Survey_date == "Sep08_2017"), ]

# QC
unique(trans_survey$Survey_date)
unique(trans_survey$Vessel)

resultsa = biomassCalc(x = trans_survey, areaKm = 830.85)
unique(resultsa$total_biomass)

map1 = mapDat(x = mapping1)
