...
poly_GB = as.PolySet(SUA, projection="LL")
...
# Seal Island
ggplot(track1,aes(x=X, y=Y)) + geom_polygon(data=out,aes(x=long, y=lat, group=group)) + coord_map() + labs(x=NULL, y=NULL)+ geom_segment(aes(x = X, y = Y, xend = Xend, yend = Yend, colour = Vessel), size = 1.2)+ geom_segment(data=track1, aes(x = X, y = Y, xend = Xend, yend = Yend, colour = Vessel), size = 1.2) + geom_segment(data=track2, aes(x=X, y=Y, xend = Xend, yend = Yend, colour = Vessel), size = 1.2) 
