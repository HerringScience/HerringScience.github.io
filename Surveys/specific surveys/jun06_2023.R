...
regions = read.table("Region_Jun06_2023.csv", header=TRUE, sep=",", row.names = "id", stringsAsFactors=FALSE)

mapping = read.table("Map_Jun06_2023.csv", header=TRUE, sep=",", row.names = "id", stringsAsFactors=FALSE)
...
SUA = read.csv("polygon_SBNorthern06.csv")
...
ids1 = c("BP_T01","BP_T02", "BP_T03", "BP_T04")
northern = trans[which((trans$RegionName %in% ids1)), ]

ids2 = c("TM_T01","TM_T02", "TM_T03", "TM_T04")
eastern = trans[which((trans$RegionName %in% ids2)), ]

ids3 =c("BP_T01","BP_T02", "BP_T03", "BP_T04", "FM_T02","FM_T03", "FM_T04" )

main = trans[which((trans$RegionName != ids3)), ]

main_ = main[which((main$RegionName != ids2)), ]
