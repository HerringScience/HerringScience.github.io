...
# GB 
regions = read.table("Region_Aug06_2023.csv", header=TRUE, sep=",", row.names = "id", stringsAsFactors=FALSE)

mapping = read.table("Map_Aug06_2023.csv", header=TRUE, sep=",", row.names = "id", stringsAsFactors=FALSE)

map = mapDat(x = mapping)
x = regions
trans = transects(x= regions, TS38 = -35.5, TS50 = NA )

SUA = read.csv("polygon_GB_06.csv")
polyGB = as.PolySet(SUA, projection="LL")

SUA = read.csv("polygon_SI_2.csv")
...
trans_survey= trans[which(trans$Survey_date == "Aug06_2023"), ]
...
SI2 = trans[which(trans$Transect_No == "T04"), ]
...
# Results
resultsa = biomassCalc(x = GB, areaKm = 511)
unique(resultsa$total_biomass)

#2,687

resultsb = biomassCalc(x = SI_, areaKm = 299)
...
tableD = resultTableA(x = SI_)
tableE = resultTableB(x = SI_)
