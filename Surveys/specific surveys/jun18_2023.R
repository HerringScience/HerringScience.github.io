...
regions = read.table("Region_Jun18_2023.csv", header=TRUE, sep=",", row.names = "id", stringsAsFactors=FALSE)

mapping = read.table("Map_Jun18_2023.csv", header=TRUE, sep=",", row.names = "id", stringsAsFactors=FALSE)
...
SUA = read.csv("polygon_SBEastern18.csv")
polySB_eastern = as.PolySet(SUA, projection="LL")

SUA = read.csv("polygon_SBNorthern18.csv")
...
ids1 = c("SL_T02","SL_T03", "SL_T04")
northern = trans[which((trans$RegionName %in% ids1)), ]

ids2 = c("LJ_T02","LJ_T03", "LJ_T04", "LJ_T05")
eastern = trans[which((trans$RegionName %in% ids2)), ]

ids3 =c("BP_T01","BP_T02", "LB_T01", "LB_T02", "LJ_T01", "SL_T01" )
...
resultsb = biomassCalc(x = northern, areaKm = 66)
b = unique(resultsb$total_biomass)

resultsc = biomassCalc(x = eastern, areaKm = 123)
