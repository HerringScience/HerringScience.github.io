...
# GB #2
regions = read.table("Region_Aug28_2022.csv", header=TRUE, sep=",", row.names = "id", stringsAsFactors=FALSE)

#regions = read.table("Region_Aug28_2022_.csv", header=TRUE, sep=",", row.names = "id", stringsAsFactors=FALSE)

mapping = read.table("Map_Aug28_2022.csv", header=TRUE, sep=",", row.names = "id", stringsAsFactors=FALSE)

#mapping = read.table("Map_Aug28_2022_.csv", header=TRUE, sep=",", row.names = "id", stringsAsFactors=FALSE)

map = mapDat(x = mapping)
x = regions
trans = transects(x= regions, TS38 = -35.45132353, TS50 = NA )
...
trans_survey= trans[which(trans$Survey_date == "Aug28_2022"), ]
...
# Results
resultsa = biomassCalc(x = GB, areaKm = 789)
unique(resultsa$total_biomass)

#7,472

resultsb = biomassCalc(x = SI, areaKm = 271)
