...
# Oct 14, 2018 German Bank
regions = read.table("Region_Oct14_2018.csv", header=TRUE, sep=",", row.names = "id", stringsAsFactors=FALSE)
mapping = read.table("Map_Oct14_2018.csv", header=TRUE, sep=",", row.names = "id", stringsAsFactors=FALSE)
...
trans_survey= trans[which(trans$Survey_date == "Oct14_2018"), ]

# QC
unique(trans_survey$Survey_date)
unique(trans_survey$Vessel)

# Remove seal island transects
# X which is > 66.3 = Seal Island

trans_survey1 <- subset(trans_survey, ( X < -66.3) )
dim(trans_survey1)
trans_survey2 <- subset(trans_survey, ( X > -66.3) )
dim(trans_survey2)
dim(trans_survey)

resultsa = biomassCalc(x = trans_survey1, areaKm =617.7865)
unique(resultsa$total_biomass)

resultsb = biomassCalc(x = trans_survey2, areaKm =22.72)
...
ggplot(plank1,aes(x=x2, y=y2)) + geom_polygon(data=out,aes(x=long, y=lat, group=group)) +  geom_polygon(data=polyGB,aes(x=X, y=Y, group=PID), colour = "black", fill="white",linetype = 3)+  geom_polygon(data=polySI,aes(x=X, y=Y, group=PID), colour = "black", fill="white",linetype = 3) + geom_point(size = 3, colour = "red") + coord_map() + labs(x=NULL, y=NULL)
