...
surv = "SB"
surv2 = "Scots Bay"
surv.date = "2026-07-05 20:00:00" #survey date and time, as formatted
surv.no = "5"
...
# 1. Download GADM data (returns a terra 'SpatVector')
# Note: geodata downloads the whole country, so we subset immediately after.
can <- gadm(country = "CAN", level = 1, path = tempdir(), version = "latest", resolution = 1)
can_sub <- can[can$NAME_1 %in% c("New Brunswick", "Nova Scotia", "Prince Edward Island", "Newfoundland and Labrador", "Québec"), ]

us <- gadm(country = "USA", level = 1, path = tempdir(), version = "latest", resolution = 1)
us_sub <- us[us$NAME_1 %in% c("Maine"), ]

# 2. Combine Canada and US subsets
NBNS_terra <- rbind(can_sub, us_sub)

# 3. Convert to 'sf' object 
# This replaces the extremely slow 'Spatial' conversion and runs instantly
NBNS <- st_as_sf(NBNS_terra)

# 4. Proper coordinates for German Bank & Crop
# st_bbox uses xmin, ymin, xmax, ymax format
GBMap <- st_bbox(c(xmin = -66.5, xmax = -65.5, ymin = 43, ymax = 44), crs = st_crs(NBNS))
GBout <- st_crop(NBNS, GBMap)

# 5. Proper coordinates for Scots Bay & Crop
SBMap <- st_bbox(c(xmin = -65.5, xmax = -64.5, ymin = 45, ymax = 45.5), crs = st_crs(NBNS))
SBout <- st_crop(NBNS, SBMap)

# 6. Make CTD Cast Box (GB or SB)
# Quotes removed to treat these as numeric values instead of character strings
...
# Make sure 'surv' is defined earlier in your script (e.g., surv <- "SB")
if (surv == "SB") {
...
c=if(surv == "SB" & is.na(EVessel) & is.na(NVessel)){
  "The Leroy and Barry and the Fundy Monarch will each complete one line in the North box and 2 lines in the main box. All other vessels complete 2 lines in the main box and one line in the East box."
...
f=if(surv=="SB" & Allocation != 0){
  paste0("7. Fishing to start on ", c, ", and allocation is ", d, 
         "8. Please wait until all vessels have completed their lines before starting to fish.")
}
