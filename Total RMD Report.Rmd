...
::: col-md-6
```{r echo=FALSE}
ggplot(data=subset(FatData, Supplier == "Unknown"), aes(x=Year, y=Fat)) +
  geom_boxplot() +
  labs(y="Fat Percent (%)")
```
:::
...
::: col-md-6
```{r echo=FALSE}
ggplot(data=subset(FatData, Supplier == "Unknown" & !is.na(Month)), aes(x=Month, y=Fat, group=Month)) +
  geom_boxplot() +
  labs(y="Fat Percent (%)")
```
:::
...
```{r echo=FALSE}
FatData %>%
  dplyr::filter(Supplier == "Unknown" & !is.na(Month)) %>%
  group_by(Month) %>%
  summarize(FatMean = mean(Fat)) %>%
  kbl(col.names = c("Month", "Avg Fat (%)")) %>%
  kable_paper("striped", full_width = F)
