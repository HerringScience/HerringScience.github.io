...
f=if(!is.na(Tagging)){
  paste("Tagging will be conducted from the ", Tagging, ".")
}
#Vessels will complete the following lines: `r a` `r b` `r c` `r d` `r e`.
```

Vessels will complete the following lines: The Morning Star will do the southern main box line and then complete the Eastern box. The Fundy Monarch will do the northern main box line (south of Isle Haute) and then complete the Northern box. All other vessels will complete two main box lines.
...
```{r}
a=if(surv=="SB"){
  paste0(PlanktonVessel, " will complete plankton tows (bottom left outer box)")
}
b=if(surv=="GB"){
...
2. Start the EK80 software and load your most up-to-date Survey settings. For NB Vessels this will be new 2023 settings, for all other vessels this will be last year's 2022 settings. When you are ready to
