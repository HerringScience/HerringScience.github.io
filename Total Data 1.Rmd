...
polysT_sf <- polysT %>%
  ...
  do({
    ...
    st_sf(FishingGround = unique(df$Box), geometry = st_sfc(st_polygon(list(mat)), crs = 4326))
  }) %>%
  ungroup() %>%
  st_as_sf()
...
r_final <- joined_spatial %>%
  st_drop_geometry() %>%
  dplyr::select(TAG_NUMBER, FishingGround) %>%
  mutate(FishingGround = case_when(
    is.na(FishingGround)       ~ "Other",
    FishingGround == "Browns Bank" ~ "Brown Bank",
    TRUE                       ~ FishingGround
  ))
# FishingGround.y is where they were initially released based on coordinates, catchAREA is where they were caught again.

TaggingEvents <- TaggingEvents %>%
  left_join(r_final, by = "TAG_NUMBER") %>%
  mutate(FishingGround = coalesce(FishingGround, "Other"))

TagReturns <- TaggingEvents %>%
  inner_join(completeReturns, by = "TAG_NUMBER") %>%
...
ggplot(data = Cat1, aes(x = catchAREA, fill = catchAREA)) +
