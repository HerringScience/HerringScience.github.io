...
r_final <- joined_spatial %>%
  st_drop_geometry() %>%
  dplyr::select(TAG_NUMBER, FishingGround.y) %>%
  mutate(FishingGround.y = case_when(
    is.na(FishingGround.y)       ~ "Other",
    FishingGround.y == "Browns Bank" ~ "Brown Bank",
    TRUE                       ~ FishingGround.y
  ))
...
ggplot(data = Cat1, aes(x = catchArea, fill = catchAREA)) +
  ...
  theme(
    ...
    axis.text.x = element_text(angle = 45, hjust = 1) # Pro-tip: tilts long area names slightly
  )
```
