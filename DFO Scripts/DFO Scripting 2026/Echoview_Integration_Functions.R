...
#Had some issues with what the lines were called so needed to create a function for the second round of exporting
# Echoview could not find the surface and bottom lines. THIS IS THE ONLY MODIFICATION TO Integration_original_2022_07_06
Integration_original_2023_02_21 <-
  function(rawFileslist,
           evFiles,
           ordered_calFiles,
           transectFiles,
    ...
    for(i in 1:length(rawFileslist)){  
      ...
      varAnaly <- EVVar[["Properties"]][["Analysis"]]
      varAnaly[["ExcludeAbove"]] <- (paste0(vessel_search[i],"-","Surface"))
