...
g=unique(ifelse(Survey$Ground == "GB", "two transects in the German Bank box and one transect in the Seal Island Box", 
         ...
         "NA"))))))
h=unique(ifelse(is.na(Survey$PlanktonVessel), "There was no plankton tow or CTD cast conducted", paste("There was a plankton tow and replicate conducted by ", unique(Survey$PlanktonVessel))))
...
`r i` vessels ran the following lines: Morning Star ran one line in the Main Box and all lines in the Eastern Box, Fundy Monarch ran one line in the Main Box and three lines in the North Box, while Brunswick Provider and Sealife II both ran two lines in the Main Box.
...
f=unique(ifelse(Survey$Sample == "Y", 
                paste0("A total of ", a, " tags were applied during the survey over ", b, " events from ", c, " to ", d, " from ", e, " vessels."), 
...
Each vessel was equipped with a Simrad ES38-18/200-18C combi (2-in-1) transducer prior to the survey that contains a 38kHz split beam and 200 kHz single beam transducer. Data was logged to the computer hard-drive for post-processing. BP and FM were calibrated on 38kHz before the survey, while all NS vessels were uncalibrated.
...
f=ifelse(Survey$Ground=="GB", paste0("Preliminary biomass prediction for German Bank using standardized parallel transects is ", b, "mt, and with turnover applied ", c, "mt. The preliminary biomass estimate for Seal Island is ", d, "mt."), paste0("Preliminary biomass prediction for Scots Bay using standardized parallel transects is ", b, "mt."))
f=f[1:1]
```
...
```{r, fig.cap="The single point with the greatest backscatter", fig.align='center'}
knitr::include_graphics(path = paste0("C:/Users/", Sys.info()[7],"/Documents/GitHub/HerringScience.github.io/Surveys/", year, "/", current, "/PRC_Snip.jpg"), rel_path = FALSE)
```

The single point with the largest backscatter density (PRC) was recorded by **`r a`**, at the following location:
...
knitr::include_graphics(path = paste0("C:/Users/", Sys.info()[7],"/Documents/GitHub/HerringScience.github.io/Surveys/", year, "/", current, "/Density_Snip.jpg"), rel_path = FALSE)
```

The transect with the overall largest biomass was recorded by **`r c`**, at the following location:
