...
Perform<-Perform %>% mutate(Start=as.POSIXct(Date.Time.Start, origin = "1970-01-01")) %>% 
  ...
  #mutate(Speed = ((Distance*1000)/(Duration))/60) Darren's code
  mutate(Speed = ((Distance*1000)/(Duration))) #Removed /60 because duration is already in seconds. Lets Speed be in m/sec
...
#Perform <- read_csv("Performance Total.csv")
